...
const guestToken = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjVmZTBkODU5ODExNTNiOWUwNWNiYjhmYyIsImFsZ29yaXRobSI6IkhTMjU2IiwiaWF0IjoxNjE0MjcxNDI5LCJleHAiOjg2NDAwMDg2NTYxNTEzNTUwMH0.xm57xoeEfryK9Qwn8_5jN2EORxO6xqjhptEpI02lCtQ";
class Globals {
    ...
    getToken(id) {
        ...
        return new Promise(async(resolve, reject) => {
            try {
                ...
                }, process.env.SECURITY_TOKEN, { expiresIn: process.env.TOKEN_EXPERY });
        ...
        return new Promise(async(resolve, reject) => {
            ...
            try {
                ...
                let token = jwt.sign({
                    ...
                    algorithm: "HS256",
                    //exp: Math.floor(new Date().getTime() / 1000) + process.env.tokenExpiry
                }, process.env.SECURITY_TOKEN, { expiresIn: flag });
        ...
        try {
            ...
            if (token) {
                ...
                if (token == guestToken) {
                    ...
                    if (!tokenExpire) return res.status(401).json({ status: 0, message: 'Token is expired.' });
    ...
    static async isEmailAuthorised(req, res, next) {
        ...
        try {
            ...
            if (token) {
                ...
                const tokenExpire = await authenticate.checkExpiration(token);
                if (!tokenExpire) return res.status(401).json({ status: 0, message: 'Your email token is expired. Please try again.' });
                next();
            } else {
                var dataErrorObj = {
                    is_from: 'API Error',
                ...
                authenticate.addErrorLogInDB(dataErrorObj)
                return res.status(401).json({ status: 0, message: 'Please send email token with api.' });
            }
