...
class File {
  ...
  store(folder, id) {
    ...
    try {
      ...
      return new Promise(async (resolve, reject) => {
        ...
        //    return resolve(fileObject);
        console.log("25454", mainfile[0], uploadedFilePath);
  ...
  bulkStore() {
    return new Promise((resolve, reject) => {
      // Setting the path
      let appDir = path.dirname(require.main.filename);
      if (_.isEmpty(this.file.file)) reject("Please send file.");
      ...
      let fileObject = {
        originalFilename: this.file.file[0].originalFilename,
  ...
  storeLocal(folder, id) {
    try {
      ...
      return new Promise((resolve, reject) => {
        ...
        let uploadedFilePath = appRoot + filePath;
        let fileObject = {
          originalFilename: newFilw[0].originalFilename,
          filePath: uploadedFilePath,
          filePartialPath: filePath,
          newName: new_name,
        };
  ...
  sendOTP(obj) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        var otp =
          obj.otp +
          " is your OTP and it is valid for the next 15 mins. Please do not share this OTP with anyone. Thankyou, ShowKIT";
        var options = {
          headers: {
            "content-type": "application/json",
          },
          ...
          url:
            "https://api.msg91.com/api/sendhttp.php?authkey=213281A44nmNY7i5ae84ee2&mobiles=" +
            obj.mobile +
            "&country=" +
            obj.country_code +
            "&message=" +
            otp +
            "&sender=TESTIN&route=4",
